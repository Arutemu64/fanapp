<script lang="ts">
	...
	async function handleSignup(e: Event) {
		...
		if (!validatePassword(password)) {
			toastService.add('Пароль должен быть от 8 до 128 символов и содержать хотя бы одну букву и одну цифру', 'warning');
			return;
		}
...
<div class="flex h-full items-center justify-center">
	<Card class="w-full max-w-md p-6 sm:p-8">
		<form onsubmit={handleSignup} class="space-y-6">
			<h2 class="mb-6 text-center text-2xl font-bold text-gray-900 dark:text-white">Регистрация в FAN App</h2>
			...
			<p class="text-center text-sm text-gray-500 dark:text-gray-400">
				Уже есть аккаунт? <a href="/login" class="text-primary-600 hover:underline dark:text-primary-500">Войти</a>
			</p>
		</form>
	</Card>
</div>