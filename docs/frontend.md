...
* **All reads go through TanStack Query.** Components fetch with `createQuery` / `createInfiniteQuery` and a generated `*Options()` helper; a `load` fetches only when the **route** must decide something a component cannot (permission guard, redirect, error page, page title), and then via `queryClient.ensureQueryData(...)` — warming the same cache entry the component reads, so there is one request and one cache. Mechanics, helper names and examples are in [api.md](api.md); the *why* is [ADR-0018](adr/0018-tanstack-query-for-frontend-data.md). All loads are **universal** (`+page.ts`/`+layout.ts`) — there are no `.server.ts` loads in a SPA.
* **The QueryClient lives on layout data, never in a module.** The root `+layout.ts` builds it once per boot and returns it, so child loads reach it via `await parent()` and components via `useQueryClient()` (the root `+layout.svelte` mounts `QueryClientProvider`). It holds the viewer's data, so a `$lib` module singleton would outlive login/logout — the rule in §1 above. Two module-scoped helpers need it anyway and take a registered closure instead of importing one: the 401 interceptor (`onUnauthorized`, `$lib/api/index.ts`) and the reconnect catch-up (`onReconnectRefresh`, `$lib/utils/reconnectRefresh.ts`).
* **Session-expiry reconciliation (401 interceptor)**: The API client carries a hey-api response interceptor that reacts to a `401` on any endpoint by refetching the identity query (debounced to one refresh per burst). It resolves to `null`, every component reading `getCurrentUserContext()` flips to the guest state at once, and the `(protected)` guard redirects on the next navigation — so an expired session flips the whole app to guest state instead of leaving a stale "logged in" UI. `/me/` itself is excluded (it *is* the verdict — reacting would loop) and so are the credential logins (`/auth/login`, `/auth/login-with-code`), whose 401 means "wrong credentials". Components keep their own inline 401 handling; the interceptor is fire-and-forget and never swallows the response.
...
* The root `+layout.ts` resolves the current user from `/me/` with `ensureQueryData` and returns `user`, so **guards** below it can decide before a route renders. Do not re-return `user` from a nested layout — it is already inherited. **Components read `getCurrentUserContext()`** (`$lib/services/currentUser.svelte.ts`) rather than `data.user`: it reads the same cache entry reactively, so a 401 or a logout updates the whole app without a route re-run.
...
* **Read-only offline data**: The whole query cache is persisted to IndexedDB (via `idb-keyval`) by `restoreQueryCache` in `$lib/api/queryClient.ts`, using `@tanstack/query-persist-client-core`. The root `+layout.ts` **awaits the restore before any route load runs**, which is what makes an offline cold boot paint real data — a query that mounted mid-restore would race it and refetch into an empty cache. Two sets decide the policy, and both default to the safe answer for an operation nobody has listed:
  * `PERSISTED_OPERATIONS` — what reaches disk (schedule, identity, subscriptions, notifications, unread count, public config). Everything else is memory-only, so a staff tool or a ballot is never served from a cold cache. A persisted query spreads `offlineQueryOptions` (a `gcTime` of 21 days — capped there because `gcTime` becomes a `setTimeout` delay and overflows a signed 32-bit int past ~24.8 days — and `networkMode: 'offlineFirst'`, so the fetch is attempted even when `navigator.onLine` lies on captive wifi, falling back to cached data when it fails).
  * `UNIVERSAL_OPERATIONS` — what carries no identity and survives logout (the programme, public config). Everything else is treated as the viewer's own and is dropped by `clearUserQueries` on logout, so an unlisted new endpoint cannot leak to the next account by omission. `api/queryClient.test.ts` guards exactly that default.

  Render `StaleDataNotice` when a query is serving a cached copy after a failed fetch (`query.isError && query.data !== undefined`), passing `query.dataUpdatedAt` so the notice shows a "synced at" time via `formatSyncedAt`. Mutations (votes, settings) stay online-only.
* **Complete-miss handling (offline vs. failure)**: A query that errored with no data at all (`isError && data === undefined`) is a cold offline boot. The page renders a calm inline "недоступно офлайн" message and keeps the app shell/bottom nav usable, and suppresses `StaleDataNotice` (there is no saved copy to caveat). A *reachable* failure is still a hard error, and that is the one thing the `load` decides: the schedule's `+page.ts` throws `error(503, …)` when the cache is empty **and** `isReachable()`, letting the offline case through to the component.
* **Online-only surfaces (`offlineUnavailable`)**: Some surfaces are deliberately *uncached* because they are all mutation and nothing worth reading offline. Voting is the read-shaped case: casting a vote is a mutation, and the open/closed and already-voted state must never be shown stale (a cached ballot you can't submit is a dead end). Its queries spread `onlineOnlyQueryOptions` (`networkMode: 'online'`, `gcTime: 0`), so offline they park as `isPaused` with nothing to show — which is exactly the signal the page reads for `offlineUnavailable`. The same flag also gates the pure submit/admin surfaces that fetch nothing: **feedback** (`feedback/+page.ts` returns `offlineUnavailable: !isReachable()`) and the whole **tools** subtree — `tools/+layout.ts` returns the flag and `tools/+layout.svelte` renders the online-only state in one place instead of gating each tool page, since every tool is an online-only mutation. Each page renders the shared `OfflineUnavailableState` component (`$lib/components/`, a fixed-icon `EmptyState` taking `title`/`message` — the copy stays per-surface because Russian agreement varies), keeping the app shell/bottom nav usable. Name the state by what its sibling encodes: an offline **miss** = a cache-backed page with nothing saved yet; `offlineUnavailable` = a surface that never caches. There is no stale copy here to caveat or timestamp.
