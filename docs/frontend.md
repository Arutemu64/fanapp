...
* **Rendering errors** are caught by SvelteKit's experimental `handleRenderingErrors` (`svelte.config.js`): each level with a `+error.svelte` is wrapped in a [`<svelte:boundary>`](https://svelte.dev/docs/svelte/svelte-boundary), so a component that throws shows the nearest error page and goes through `handleError`. Add a local `<svelte:boundary onerror={…}>` only for a widget that sits *above* a route's error page — in a layout shell, like `NotificationBell` in `AppNavbar` — where the nearest boundary would otherwise take the whole shell down; report the error to Sentry in `onerror`, since a local boundary never reaches `handleError`. Boundaries do not catch event handlers or async work, so API failures still go through the error funnel ([api.md](api.md)).
