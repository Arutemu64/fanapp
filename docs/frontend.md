...
This NGINX container is also the stack's **single web edge**: besides the SPA it reverse-proxies `/api` to the backend `api` service (stripping the `/api` prefix; the backend runs with `UVICORN_ROOT_PATH=/api`), so the browser sees one origin — no CORS, first-party cookies. `/api/events` (SSE) gets its own unbuffered, keep-alive location. In dev the Vite server does the same `/api` proxy (`vite.config.ts`, target `BACKEND_ORIGIN`), so `PUBLIC_API_URL` is just the relative path `/api` in both. The upstream Caddy still terminates TLS in front of this one port.
