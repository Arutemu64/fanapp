...
* **Filter state belongs in the URL**: search boxes and filter toggles (schedule, voting nomination) hold their state in query params via `UrlParam` / `UrlFlag` (`$lib/utils/urlState.ts`), so a reload, a shared link or a there-and-back navigation restores the same view. Construct one during component init and bind straight to it — `bind:value={search.current}`. The URL is the single source of truth: `current` reads `page.url`, so a change from outside (Back/Forward, a link carrying the param) needs no syncing. Two constraints are baked in and worth knowing before you reach for something simpler:
