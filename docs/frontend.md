...
* **The precache is not simply `build`**: `ASSETS` filters out font subsets the app can never render (Greek, Vietnamese, and the display face's Latin-ext). `unicode-range` keeps the *browser* from fetching those, but it has no say over what the SW pulls at install, where precaching `build` wholesale cost ~170 KB per install. The faces stay declared in `app.css`, so a stray glyph still renders online and the fetch handler's opportunistic `cache.put` keeps whatever gets used — changing fonts degrades this list gracefully rather than breaking it. Verify a change by inspecting the SW cache in DevTools (Application → Cache Storage), not by reading `build`: the filter runs at runtime.
...
Two fonts are defined in `app.css`: `font-sans` (Inter, body text) and `font-display` (Unbounded, hero headings and countdown numerics only). Both are imported whole from Fontsource; `unicode-range` in those packages means the browser only ever downloads the subsets a page actually renders (4 of 11 files on the home page). Which subsets get **precached** is a separate decision and lives in `src/service-worker.ts` — see §2. Apply sizes by role:
