...
* **Connectivity vs. stream health**: `OfflineService` (`$lib/services/offline.svelte.ts`) tracks `navigator.onLine` via `createSubscriber`; `ConnectionBanner` shows the offline strip from it, distinct from the SSE `EventsClient` reconnect state. `EventsClient` also runs a liveness watchdog: the backend emits a named `ping` event on stream silence, and the client reconnects if nothing arrives for `HEARTBEAT_TIMEOUT_MS` (see "Realtime (SSE)" in [backend.md](backend.md)).
