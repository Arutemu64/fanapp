...
* **Type-aware rules are on** — `typescript-eslint`'s `recommendedTypeChecked` preset, resolved through `projectService`. So `no-floating-promises`, `no-misused-promises` and the `any`-safety rules are live: an un-awaited promise is a lint error, not a review catch. Two consequences worth knowing:
  * **A file outside the SvelteKit TS project must be added to the carve-out**, or linting it fails with a parser error rather than a rule violation. The last config block lists them (`eslint.config.js`, `svelte.config.js`, `scripts/**`, `src/service-worker.ts` — SvelteKit deliberately excludes the service worker from the app tsconfig) and applies `ts.configs.disableTypeChecked`. Add new root-level configs or build scripts there too.
  * **`extraFileExtensions: ['.svelte']` is declared globally**, not just in the Svelte override. With `projectService`, differing values across files in one TS project force a full project reload per file — keeping it consistent is what takes a cold lint from ~2min to ~15s. Don't "tidy" it into the Svelte block.
* **`no-useless-assignment` is off for Svelte files**: it false-positives on `$bindable` props, seeing the write-out to the parent but not the parent-side `bind:` read ([sveltejs/eslint-plugin-svelte#1478](https://github.com/sveltejs/eslint-plugin-svelte/issues/1478)). Re-enable once that lands.
