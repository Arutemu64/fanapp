...
* **Stale-while-revalidate (`fetchWithCacheSwr`)**: `fetchWithCache` is network-first, so on a slow link a page with a perfectly good saved copy still waits up to `FIRST_PAINT_TIMEOUT_MS` before painting. `fetchWithCacheSwr` takes the same options but, when a cached copy exists and the server looks reachable, returns it at once (`stale: true`) and hands back the network refresh as `revalidated` — a promise that never rejects (a failure resolves to the cached copy, `stale: true`), runs under the longer `REVALIDATE_TIMEOUT_MS` since nothing waits on it, and resolves only once the fresh copy is persisted, so a `load` re-run right after reads it rather than the copy it replaced. With no cached copy, or offline, it behaves exactly like `fetchWithCache` and `revalidated` is `null`. The schedule uses it ([web.dev on SWR](https://web.dev/articles/stale-while-revalidate)): `+page.ts` returns the saved view plus `revalidated`, and `+page.svelte` keeps a writable `$derived` view that an `$effect` overwrites when the promise resolves (the cleanup drops a revalidation a newer `load` superseded). A copy being revalidated does **not** raise `StaleDataNotice` — its "нет связи" copy would be false while the refresh runs; a failed revalidation raises it. Because every `invalidate('app:schedule')` now resolves off IndexedDB, read-your-writes callers (`SubscribeModal`, `EventCard`) see their change when the revalidation lands, one round-trip after `invalidate` resolves, still without waiting on the SSE echo. Only use it where a stale copy is harmless to show first; voting and `/me` stay as they are.
