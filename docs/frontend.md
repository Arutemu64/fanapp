...
* **`manifest.json` shell colours are palette-derived, not brand-literal.** `theme_color` is `#960d3a` (`primary-800`) — a deliberately *darkened* brand shade, not the `primary-600` action colour, so the OS status-bar/task-switcher tint stays gentle in a dark venue. `background_color` is `#111827` (`gray-900`, matching `dark:bg-gray-900` on `<body>` and the `app.html` boot splash) so the Android launch screen doesn't flash white in a dark hall. The manifest allows only one value each, so both are tuned for the dark-mode majority rather than made scheme-aware — only the `app.html` `theme-color` metas can vary by `prefers-color-scheme`. On rebrand, re-derive both from the new palette (deepen `theme_color`, match `background_color` to the app's dark surface).
