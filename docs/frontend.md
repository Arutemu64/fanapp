...
* **Type-aware rules** (`typescript-eslint` `recommendedTypeChecked`) **are** enabled, via `projectService`. `extraFileExtensions: ['.svelte']` is declared globally rather than only in the Svelte override — differing values across files in one TS project force a project reload per file, which is the difference between a ~15s and a ~2min cold lint. Files outside the SvelteKit TS project (root configs, `scripts/**`, `src/service-worker.ts`) opt out through `ts.configs.disableTypeChecked`.
* **Complexity budget**: `complexity` (12) and `max-depth` (4) run as **warnings**, giving the frontend the rough equivalent of what ruff's mccabe check already gives the backend. They are warnings on purpose — a high score is a prompt to read the function, not a defect. Some handlers are branchy because the API they drive is: `PushNotificationsCard`'s subscribe flow is flat, guard-clause code and still scores 21, because the Web Push handshake has that many failure points. Erroring would only buy inline disables. Split a function when the score reflects *incidental* nesting; leave it and move on when the branches are the domain.
