...
* **Filters live in the query string (`$lib/utils/urlFilters.ts`)**: A page's search box and filter toggles mirror into the URL (`?q=`, `?subscribed=1` on the schedule; `?q=` per nomination in voting), so a filtered view survives the three things that drop component state — the self-reload when a new service worker takes over, a trip to a sub-page and back, and a link pasted to someone else. The mirror is **one-way**: the filter is seeded from `page.url` once at component init and owned as `$state` from then on. Do **not** read it in `load` instead — SvelteKit tracks each search param a `load` reads, so that turns every keystroke into a re-run. `createUrlFilterSync()` writes back with `goto(…, { replaceState, keepFocus, noScroll })`: `replaceState` so a typed word doesn't leave one history entry per character, `keepFocus` so the caret stays in the search box. Call it during component init (it cancels its pending write on destroy — a debounced `goto` that outlives its page navigates the user back to it), and drive it from the control's handler or a **function binding**, never a state-syncing `$effect`. Flowbite's `Search` clears by writing `undefined` rather than `''`, so a setter taking the value must normalize it.
