...
* **Telegram OAuth outcomes (`?telegramLoginError=` / `?telegramLinkError=`)**: both Telegram callbacks are entered by a top-level browser navigation, so the backend cannot answer them with an error body — the browser would render the JSON as the page. Instead it redirects to `/login` or `/profile` with a one-time code. `$lib/utils/telegramOAuth.ts` holds the param names, the shared `cancelled`/`failed` codes (mirrored from `presentation/web/telegram_oauth.py`) and `readTelegramErrorCode`, which whitelists the value — never render a code straight off the URL. Each page maps its codes to Russian copy and clears the param with `clearTelegramErrorParam` so a reload does not replay the toast.
