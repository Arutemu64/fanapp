...
* **Browser globals**: `window`, `document`, and `localStorage` are safe in component and module code, since nothing runs on the server. (Still guard with `browser` from `$app/environment` only if a module also runs under non-browser tooling, e.g. tests.)
...
* **Filter state belongs in the URL**: search boxes and filter toggles (schedule, voting nomination) hold their state in query params via `UrlParam` / `UrlFlag` (`$lib/utils/urlState.svelte.ts` — runes only compile in `.svelte.ts` modules), so a reload, a shared link or a there-and-back navigation restores the same view. Construct one during component init and bind straight to it — `bind:value={search.current}`. The URL is the single source of truth: `current` reads `page.url`, so a change from outside (Back/Forward, a link carrying the param) needs no syncing. Two constraints are baked in and worth knowing before you reach for something simpler:
  * **Write with `goto`, not shallow routing's `replaceState`.** `replaceState` updates the address bar but leaves `page.url` untouched (it only assigns `page.state`), so anything written with it can never be read back — and a "clear the filter" write silently no-ops against the stale `page.url`. `goto(url, { replaceState: true, keepFocus: true, noScroll: true })` is what the [SvelteKit state docs](https://svelte.dev/docs/kit/state-management) recommend for filters; `replaceState: true` keeps them out of the back button, and `keepFocus`/`noScroll` stop a navigation per keystroke from pulling focus out of the search box or jumping the page to the top.
  * **The write is debounced, the value is not.** Typing updates `current` immediately from a small pending buffer and the URL follows ~300ms later, so the input never lags. Clearing the buffer waits for `goto` to resolve — dropping it earlier flashes the pre-navigation value back into the field.

  Transient state (a picker inside a modal) stays out of the URL. If this ever needs typed/coerced params, defaults or multi-param batching, replace it with [`sveltekit-search-params`](https://github.com/paoloricciuti/sveltekit-search-params) rather than growing it.
