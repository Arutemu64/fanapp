...
* **Read-only offline data**: Pages worth viewing offline (programme, notifications, the home hero's festival config; the profile renders from the cached `/me` identity below) read through **TanStack Query**, persisted to IndexedDB. Create the query in the *component*, from the generated options in `$lib/api/queries` (see [api.md](api.md)), wrapped in `persisted(options, scope)` from `$lib/query/persist.ts` — persistence is opt-in, so an uncached surface stays uncached by default. `scope` mirrors the old cache scopes: `'user'` entries (subscriptions, the notification feed) are removed on logout so one device's account never serves another's data; `'universal'` entries (the public programme, the config) are identical for everyone and survive logout. Derive the page's offline flags with `offlineQueryState(query, offline.isOnline)` (`$lib/query/offlineState.ts`) — it returns `{ stale, cachedAt, offlineMiss }`, the same shape the loads used to. Render `StaleDataNotice` when `shouldShowStaleNotice(...)` and pass `cachedAt` (epoch millis of the shown copy's fetch) so the notice shows a "synced at" time via `formatSyncedAt`. Mutations (votes, settings) stay online-only; after one succeeds, call the matching helper in `$lib/query/invalidate.ts` (`invalidateSchedule`, `invalidateSubscriptions`, …) instead of `invalidate('app:…')`. Identity is the one exception: `/me` keeps its `load` and its own `fetchWithCache` entry, because `(protected)/+layout.ts` must resolve a user before it can guard and a `load` cannot reach the QueryClient. See [ADR-0018](adr/0018-tanstack-query-for-cached-read-surfaces.md).
* **Loading, miss and failure states live on the page**: a migrated page's `load` no longer fetches, so first paint is the component's job. Branch in this order — `isPending && data === undefined` renders the section skeleton the app shell used to show during navigation (`ScheduleSkeleton`, `NotificationsSkeleton`); `offlineState.offlineMiss` (offline, nothing saved) renders a calm inline "недоступно офлайн" message and keeps the app shell/bottom nav usable; `isError && data === undefined` (a *reachable* failure) renders an error state with a «Повторить» button wired to `query.refetch()`, rather than throwing a 503 into the full error page. `StaleDataNotice` is suppressed on an offline miss — there is no saved copy to caveat.
...
* **Warming a cache before first visit**: the root layout's `warmOfflineQueries` prefetches the programme (and the signed-in user's subscriptions) so they work offline from the first run. It is wired to `PersistQueryClientProvider`'s `onSuccess`/`onError`, i.e. it runs once the persister has finished restoring — so a copy that was already saved is never refetched at launch. Fire-and-forget, and a no-op offline because the query pauses; the page's own query plus the `schedule_updated` SSE event keep it fresh after that.
* **Freshness, pausing and busting**: the client's defaults live in `$lib/query/client.ts` — a `staleTime` that replaces the schedule page's old hand-rolled visibility throttle, and a `gcTime` that must stay at least the persister's `maxAge` (a garbage-collected query is gone from the next snapshot). `bindOnlineManager` (`$lib/query/online.ts`) drives TanStack's online state from our backend reachability probe instead of `navigator.onLine`, so on a captive portal or dead VPN queries *pause* and keep showing the restored copy rather than fetching into a black hole. A change that makes old snapshots unreadable must bump `PERSISTED_CLIENT_BUSTER` in `$lib/query/persister.ts`; it is deliberately not the build hash, because busting on every deploy would empty the offline cache of anyone who updates the PWA while away from the network.
