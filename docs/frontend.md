...
The app is an installable PWA: `static/manifest.json` (icons, standalone display) + a service worker (`src/service-worker.ts`, the SvelteKit `$service-worker` template) that the root layout registers via `registerServiceWorker` (`$lib/utils/serviceWorker.ts`). SvelteKit's own auto-registration is disabled (`serviceWorker.register: false` in `svelte.config.js`) because it leaves the `register()` promise's rejection unhandled — a browser that refuses registration (storage-partitioned in-app browsers, private modes) degrades to online-only, which is expected, but surfaced an uncaught "Error: Rejected" to Sentry; our wrapper mirrors SvelteKit's defaults (same script path and module/classic type, registered on `load`) with a `.catch()`. The SW precaches the app shell (the non-image `build` output — JS/CSS/fonts, plus the SVG logo/favicon — and everything in `static/`; responsive raster image sets are runtime-cached instead, see below), handles web-push, and **never caches API requests** — the backend is served under a path on the *same* origin (e.g. `/api`, derived from `PUBLIC_API_URL`), so the SW excludes it by path (`isApiRequest`), not by origin; its user-specific data is cached by the app layer, never the SW. `UpdatePrompt.svelte` (mounted in the root layout) surfaces a "new version" banner when a fresh build is waiting and reloads on `controllerchange`; the SW activates the waiting worker only when it receives a `'skipWaiting'` message. OS integration: the manifest declares `shortcuts` (long-press app-icon menu: Программа/Уведомления/Карта) and `launch_handler: navigate-existing` (links into the installed app reuse the open window instead of spawning a duplicate), and the Badging API mirrors the unread-notification count onto the app icon via `$lib/utils/appBadge.ts` — `NotificationBell` syncs the exact count, logout clears it, and the SW push handler sets a count-less "flag" badge that the bell replaces on next open.
...
* **Online-only surfaces (`offlineUnavailable`)**: Some read surfaces are deliberately *uncached* — voting (`voting/+page.ts`, `voting/[nominationCode]/+page.ts`) is the case: casting a vote is a mutation, and the open/closed and already-voted state must never be shown stale (a cached ballot you can't submit is a dead end). Their loads mirror the complete-miss shape without a cache behind it: skip the request when `!isReachable()`, and in the `catch` re-throw a `throwApiError` result (an `isHttpError` — a *reachable* failure keeps its mapped status) but treat any other throw as a network failure — `markReachable(false)` and return an `offlineUnavailable: true` flag. The page renders a dedicated "доступно только онлайн" empty state, keeping the app shell/bottom nav usable. Name the flag by what the sibling encodes: `offlineMiss` = a cache-backed page with nothing saved yet; `offlineUnavailable` = a surface that never caches. There is no `stale`/`cachedAt` here — there is no saved copy to caveat or timestamp.
* **Network failures are typed, not sniffed (`NetworkError`)**: `createApiClient` registers a `networkFailure` middleware on openapi-fetch's [`onError`](https://openapi-ts.dev/openapi-fetch/middleware-auth) hook — the one place the library surfaces a request that never produced a response (connection dropped, CORS refused, aborted). It re-throws the raw rejection as a `NetworkError` (`$lib/api/errors.ts`), so callers branch on `isNetworkError(err)` instead of matching a message that differs per browser ("Failed to fetch" in Chrome, "NetworkError when attempting to fetch resource." in Firefox). The middleware only *labels* the failure — it deliberately still throws, because what an outage costs differs per call site (serve the cached copy, show "только онлайн", show the offline page). The corollary matters as much: a `catch` that treats *any* throw as an outage hides genuine bugs behind a page blaming the user's connection, so branch on the type and re-throw the rest.
* **Online-only staff pages (`loadOnline`)**: The operational pages — festival settings, sync, the feedback and schedule-change feeds — are uncached for the same reason as voting, but have no calm empty state to fall back to: an organiser needs the live numbers or nothing. Wrap the request in `loadOnline` (`$lib/api/errors.ts`) instead of hand-rolling the `catch`. It skips the request when `!isReachable()`, re-throws anything that is not a `NetworkError` (a `throwApiError` result, a `redirect`, a real bug), and turns an outage into `markReachable(false)` plus the same `error(503, 'Нет связи с сервером')` the `(protected)` layout throws, which `ErrorState` reframes into the offline page and `hooks.client.ts` keeps out of Sentry. Unguarded, that rejection escapes the load as an unhandled crash and is filed as an app bug — this is [what to fix at the source rather than silence with Sentry's `ignoreErrors`](https://swiftmade.co/blog/2026-01-05-sentry-error-reporting-best-practices/), which would also mask the CORS breakage that presents identically. It is the wrong tool wherever the page can show *something* offline: those want `fetchWithCache` + `offlineMiss`, or the `offlineUnavailable` flag above.
