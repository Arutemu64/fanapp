...
* **Relative API base (`PUBLIC_API_URL=/api`)**: The default is a **relative** path, not an absolute URL. The SPA and API are served same-origin (Caddy routes `/api*` to the backend), so a relative base resolves against whatever origin serves the app — keeping the bundle (and the prebuilt GHCR image) domain-agnostic, so the same build runs on any domain with no rebuild. Browser consumers (the hey-api fetch client, the reachability probe, the SSE `EventSource`, the Telegram OAuth `href`s) resolve it against `location.origin`; the service worker resolves it against its own origin (`new URL(PUBLIC_API_URL, self.location.origin)`) since `new URL` throws on a bare path. In dev the frontend and backend are different origins, so `frontend/vite.config.ts` proxies `/api` to the backend (`VITE_API_PROXY_TARGET`, default `http://localhost:8000`; `http://api:8000` in the Docker dev overlay) and strips the `/api` prefix to mirror Caddy. Set an absolute URL only for the opt-in split-origin deployment (then also set `WEB__CORS_ALLOW_ORIGINS`).
...
* **Every read goes through TanStack Query.** There is one `QueryClient` and one cache; nothing fetches around it. See [ADR-0018](adr/0018-tanstack-query-for-frontend-data.md) for why. The client is created per app boot in the root `+layout.ts` and returned as `data.queryClient` — never a module singleton, since it holds the signed-in user's data and modules outlive login/logout in this SPA. Child loads reach it with `await parent()`; components use `useQueryClient()`.
* **Options come from the generated helpers, not hand-written keys.** The Hey API `@tanstack/svelte-query` plugin emits a `*Options()`, `*QueryKey()` and `*Mutation()` per operation into `$lib/api/generated/@tanstack/svelte-query.gen.ts`. Pass them straight in — `createQuery(() => getScheduleOptions())`, `queryClient.invalidateQueries({ queryKey: getScheduleQueryKey() })`. Never build a `queryKey` by hand: the generated one encodes the operation's params, so a hand-rolled key silently misses invalidations. Two places wrap a generated helper, both for a stated reason: `$lib/api/session.ts` (401/403 is the *guest* answer, not a failure) and `$lib/api/feeds.ts` (page params, below).
* **Loads prefetch, components read.** A `load` calls `queryClient.prefetchQuery(...)` (or `prefetchInfiniteQuery`) and returns at most a `title` and route params; the component reads the same options with `createQuery`. Keeping the route in the load preserves nav-blocking, guards and error pages, while the component gets live updates from invalidation. `prefetchQuery` never throws — `await` it only where first paint must not show stale data (the home page's festival phase) or where the load needs the response (the user-detail page's navbar heading, via `ensureQueryData`); otherwise leave it fire-and-forget (`void`) so first paint is never blocked and the component renders its skeleton.
* **Svelte adapter shape**: options are a **thunk** — `createQuery(() => ({ … }))`, not `createQuery({ … })` — so reactive reads inside are tracked. The returned value is a **proxy and cannot be destructured**; read `query.data`, `query.isPending`, `query.dataUpdatedAt` directly, or wrap in `$derived`. See the [Svelte Query overview](https://tanstack.com/query/latest/docs/framework/svelte/overview).
* **One configured client, no per-call plumbing.** The generated module-level client is configured once by `$lib/api/heyApiConfig.ts` (Hey API's `runtimeConfigPath`) with the API base URL and `credentials: 'include'`, so no call site constructs or threads a client. There is no `createApiClient` any more, and SDK calls take no `client` or `fetch` argument. TanStack owns request deduplication and cancellation (the queryFn gets an `AbortSignal`), which is what passing the load `fetch` used to buy.
* **Session-expiry reconciliation (401 interceptor)**: `attachSessionReconciler` (`$lib/api/index.ts`, called from the root load) installs a hey-api response interceptor that reacts to a `401` on any endpoint by invalidating `getCurrentUserQueryKey()`. The identity query refetches, resolves to a guest, and the `(protected)` guard redirects — so an expired session flips the whole app to guest state instead of leaving a stale "logged in" UI. A burst of rejected calls collapses on its own, because `invalidateQueries` dedupes concurrent refetches of the same key. `/me/` itself is excluded (it *is* the verdict — reacting would loop) and so are the credential logins (`/auth/login`, `/auth/login-with-code`), whose 401 means "wrong credentials". Components keep their own inline 401 handling; the interceptor is fire-and-forget and never swallows the response. It is an interceptor rather than a `QueryCache` error handler because the decision needs the HTTP status and the operation path, and the SDK's `throwOnError` throws the parsed error body alone.
...
* **Read-only offline data is the persisted query cache.** The whole cache is dehydrated to IndexedDB by `@tanstack/query-persist-client-core` + `idb-keyval` (`$lib/api/persister.ts`), restored by `PersistQueryClientProvider` in the root layout before children render, so a page paints from its last synced data and *then* revalidates. Two settings carry this and must not be lowered casually: `gcTime` is 24h (a query garbage-collected during the session is missing from the dehydrated snapshot, so it would not be there on the next offline boot) and `networkMode: 'offlineFirst'` (a paused query renders nothing — the route must serve its saved copy *and* still attempt the network). `persistOptions.maxAge` must never exceed `gcTime`, or a restored query is evicted before anything reads it. Render `StaleDataNotice` while offline with data, passing `query.dataUpdatedAt` so the notice shows a "synced at" time via `formatSyncedAt`. Only **one** IndexedDB database may be opened: two idb-keyval databases opening concurrently race in Firefox and leave the second permanently unusable (idb-keyval issue #32), so share `persister.ts`'s store rather than calling idb-keyval's default `get`/`set`. Mutations (votes, settings) stay online-only.
* **Complete-miss handling (offline vs. failure)**: nothing persisted and offline is a different state from a failed request. Derive it in the component — offline **and** the query has no data — and render a calm inline "недоступно офлайн" message so the app shell and bottom nav stay usable; a route whose load hard-fails still throws `error(503, …)` and gets `ErrorState`. Suppress `StaleDataNotice` on a cold miss: there is no saved copy to caveat.
* **Keeping something *out* of the persisted cache** is `gcTime: 0` on that query, set at every call site that reads it (the load's prefetch and the component's `createQuery`). Voting uses this — see the online-only bullet below.
* **Online-only surfaces (`offlineUnavailable`)**: Some surfaces are deliberately *uncached* because they are all mutation and nothing worth reading offline. Voting (`voting/+page.ts`, `voting/[nominationCode]/+page.ts`) is the read-shaped case: casting a vote is a mutation, and the open/closed and already-voted state must never be shown stale (a cached ballot you can't submit is a dead end). Its queries carry `gcTime: 0`, so nothing about them is persisted and an offline visit genuinely has nothing to show; the component derives `offlineUnavailable` from "offline, or the query errored with no data". The same flag also gates the pure submit/admin surfaces that fetch nothing: **feedback** (`feedback/+page.ts` returns `offlineUnavailable: !onlineManager.isOnline()`) and the whole **tools** subtree — `tools/+layout.ts` returns the flag and `tools/+layout.svelte` renders the online-only state in one place instead of gating each tool page, since every tool is an online-only mutation. Each page renders the shared `OfflineUnavailableState` component (`$lib/components/`, a fixed-icon `EmptyState` taking `title`/`message` — the copy stays per-surface because Russian agreement varies), keeping the app shell/bottom nav usable. Name the flag by what the sibling encodes: `offlineMiss` = a page whose data persists but has nothing saved yet; `offlineUnavailable` = a surface that never caches.
...
* **Logout works offline, revoke is queued (`pendingLogout`)**: the session lives in an HttpOnly cookie, so JS can't clear it — a purely local logout would let the still-valid cookie silently restore the session on the next `/me`. So `AppNavbar.handleLogout` branches on `offline.isOnline`: online logs out normally; offline calls `markLogoutPending()` (`$lib/utils/pendingLogout.ts`, a persisted intent in `safeStorage`) then tears down local state (`queryClient.clear()` + `idbPersister.removeClient()`, badge, guest nav) at once so a shared device stops showing the account. The queued `POST /auth/logout` fires from `OfflineService` — on the offline→online edge and on a fresh boot (a boot that is already online never crosses the edge) — and is idempotent, so it is one intent, not a write queue. Until it succeeds, the root `+layout.ts` gates identity on `isLogoutPending()` (returns `{ user: null }`) so a valid cookie can't resurrect the account and there's no "logged back in" flicker. A fresh login cancels the stale intent: `completeLogin` clears it for password/code, and the VK OAuth click clears it before navigating away (OAuth bypasses `completeLogin`).
* **Identity contract (`currentUserQueryOptions`, `$lib/api/session.ts`)**: the `/me` queryFn distinguishes three outcomes so a transient error can't silently log a user out: `200` → the user; explicit **`401`/`403`** → resolve as **`null`**, the cacheable guest state (thrown, it would never persist, and an offline boot could not tell "guest" from "unknown"); any **other** error (`5xx`, parse, empty) → **throw**, so the root load keeps the last-good cached identity rather than downgrading it. `retry: false`, since a retry only delays that fallback. It reuses the generated `getCurrentUserQueryKey()` verbatim, which is what lets the 401 interceptor invalidate it from anywhere.
* **Warming a cache before first visit**: the root `+layout.ts` fire-and-forgets `prefetchQuery` for the schedule (and subscriptions, when signed in) so those routes work offline from the first run even if the user never opens them. `staleTime` makes it a no-op once warm, and the page's own prefetch deduplicates against it rather than issuing a second request — the whole "already cached? skip" bookkeeping the old `warmCache` needed is now `staleTime`.
* **Paginated feeds are infinite queries.** Notifications, schedule changes, tools feedback and broadcast history each request `PAGE_SIZE + 1` items and display `PAGE_SIZE`; the extra item is the only signal a next page exists, because these endpoints return no total. `offsetPageParams` (`$lib/api/pagination.ts`) turns that convention into `initialPageParam`/`getNextPageParam`, and `flattenPages` drops each page's probe item. **The generated `*InfiniteOptions()` helpers do not include page params** — every caller must merge them in, and a route that prefetches with the bare generated options writes a cache entry with no `getNextPageParam` that throws when a component reads it. So each feed has exactly one builder in `$lib/api/feeds.ts` (`notificationsFeedOptions()`, …) used by the load, the component and every invalidation; do not call `*InfiniteOptions()` directly outside that file.
* **Connectivity vs. stream health**: connectivity is TanStack Query's `onlineManager` — the same signal that decides whether a query may run, so the banner can never disagree with what the data layer is doing. Read it reactively through `online.current` (`$lib/services/online.svelte.ts`, a `createSubscriber` bridge) in components rendered outside the app shell, such as `ErrorState`; `onlineManager.isOnline()` is the plain read for `load` functions, which run outside any effect. `OfflineService` (`$lib/services/offline.svelte.ts`) exposes it as reactive state and owns the two side effects a recovery owes: flushing a logout queued while offline, and a debounced `invalidateQueries()` catch-up (`$lib/utils/reconnectRefresh.ts`, shared with the SSE reconnect path so one recovery can't trigger a refetch storm). **This is `navigator.onLine`**, which reports online on a captive portal or a connected-but-dead VPN; the app's copy says "Нет интернета" and does not claim to tell a dead device connection from an unreachable backend. `ConnectionBanner` shows the strip from it, distinct from the SSE `EventsClient` reconnect state. The banner is designed not to be distracting on flaky cellular: it waits `RECOVERING_GRACE_MS` (8s) before showing the "reconnecting" strip (so a normal SSE reconnect after foregrounding resolves before the user sees anything), enforces a `MIN_DISPLAY_MS` (4s) minimum once shown (so it never flashes on and off), and slides in/out instead of popping. `getEventsClient()` returns a non-nullable client — the root layout always constructs one — so subscribe with `eventsClient.on(...)`, not `?.on(...)`. `EventsClient` also runs a liveness watchdog: the backend emits a named `ping` event on stream silence, and the client reconnects if nothing arrives for `HEARTBEAT_TIMEOUT_MS` (see "Realtime (SSE)" in [backend.md](backend.md)). Its reconnect policy never gives up outright — after `MAX_RECONNECT_ATTEMPTS` fast retries it shows the "Соединение потеряно" banner but keeps dialing every `FAILED_RETRY_INTERVAL_MS`, because a stream that breaks while the device stays online produces no connectivity transition for the other recovery paths to hook.
* **SSE events invalidate, they never write the cache.** A page subscribes to its `SSEEventName` and calls `queryClient.invalidateQueries({ queryKey })`; it subscribes to `connection_established` with the same handler, so an event missed while the stream was down self-heals on reconnect. Treat the SSE payload as a nudge to refetch, never as the source of truth — splicing it into the cache reorders paginated feeds and desynchronises counts the server owns.
