...
* **Online-only staff pages (`loadOnline`)**: The operational pages — festival settings, sync, the feedback and schedule-change feeds — are uncached for the same reason as voting, but have no calm empty state to fall back to: an organiser needs the live numbers or nothing. Wrap the request in `loadOnline` (`$lib/api/errors.ts`) instead of hand-rolling the `catch`. It skips the request when `!isReachable()`, passes a `throwApiError` result or a `redirect` straight through (a server that answered gave a verdict), and converts anything else — a network throw — into `markReachable(false)` plus the same `error(503, 'Нет связи с сервером')` the `(protected)` layout throws, which `ErrorState` reframes into the offline page and `hooks.client.ts` keeps out of Sentry. Without it the raw `TypeError` openapi-fetch re-throws on a dropped connection escapes the load as an unhandled crash, filed as an app bug. It is the wrong tool wherever the page can show *something* offline: those want `fetchWithCache` + `offlineMiss`, or the `offlineUnavailable` flag above.
