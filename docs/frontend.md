...
* **Filter state belongs in the URL**: search boxes and filter toggles (schedule, voting nomination) mirror their state into query params via `syncUrlParams` (`$lib/utils/urlState.ts`), so a reload, a shared link or a there-and-back navigation restores the same view. The pattern is a one-way seed — `$state` initialized from `page.url.searchParams` once — plus a **debounced** `$effect` that writes back; browsers reject `replaceState` called too often, and a write per keystroke also parks half-typed queries in the address bar. It uses `replaceState`, not `pushState`, so filters never flood the back button. Transient state (a picker inside a modal) stays out of the URL.
