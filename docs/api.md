...
* **Reachability watch**: records backend reachability from each HTTP outcome (via `reportRequestReachability`). A response marks us reachable — even a 4xx/5xx body proves the backend answered — except a gateway 5xx (502/503/504), which counts as a failure like a network throw. A thrown error marks us unreachable, unless it is a bare navigation `AbortError` (a superseded `load`, not an outage). So `load`/cache code no longer calls `markReachable` itself; it only reads `isReachable()` to skip doomed requests. **Concurrency**: because several requests run per navigation (e.g. the notification preview alongside its unread count), a *failure* is suppressed when another request has succeeded within the timeout budget — a successful response is authoritative and must win over a slow sibling that fails just after. A genuine outage fails every request, so nothing suppresses the `false`, and the health probe (which bypasses this guard) remains the arbiter.
