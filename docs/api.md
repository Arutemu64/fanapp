...
* **Checking a mutation result**: use `getApiFailureMessage(error, response, fallback)` (`lib/api/errors.ts`) — it returns the Russian message when the call failed and `null` when it succeeded, so one `if` replaces the `error || !response.ok` pair. Testing only `error` silently treats a non-2xx with no parsed body (an empty 500, a proxy error page) as success. It resolves the message but deliberately does not act on it: whether a failure becomes an inline field error, a toast or an optimistic revert stays at the call site. Where the field a failure belongs to depends on the status, pick the *fallback* by status and let the shared copy still win — see `ChangeEmailModal.svelte` (409 → the email field). Never hand-roll a status→message ladder; that is what the dictionary is for.
* **Single funnel**: `getApiErrorDetail(error)` (`lib/api/errors.ts`) turns a payload into Russian copy, and backs both the helper above and `toastService.error(err)`. For **`load` failures** use `throwApiError(apiError, response, fallback)` from the same module — it throws a SvelteKit `error()` with the mapped copy and the real HTTP status (and carries `code` on `App.Error`), so a load failure looks like every other error instead of a bespoke per-page string. Add new copy to the `ERROR_MESSAGES` dictionary there. (Permission guards and offline-cache-miss states still throw `error()` directly — they have no API `code` to map.)
