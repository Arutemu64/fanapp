...
* **Shared base & mixins** (`models/base.py`, `models/mixins/`): `BaseORM` holds the shared `metadata` (with the `ix`/`uq`/`ck`/`fk`/`pk` naming convention that the constraint-violation handling relies on), the universal `created_at` audit column, and a generic `__repr__`. Compose the rest from mixins rather than re-declaring per model:
  ...
  * `UpdatedAtMixin` (`models/mixins/timestamps.py`) — the `updated_at` column. **Opt-in**: mix it into a model only if the table actually receives UPDATEs. Append-only / audit tables (`votes`, `feedback`, `schedule_changes`, `user_flags`, `user_permissions`, `push_subscriptions`, `social_identities`) omit it — an `updated_at` that can never move is just noise. (`created_at` stays everywhere; only `updated_at` is opt-in.)
...
* **OAuth flow-start endpoints are POST + origin-checked (`presentation/web/csrf.py`)**: `POST /auth/login/telegram` and `POST /me/connections/telegram` begin the Telegram handshake. They are **not** GET, and must not be turned back into one: a GET flow-start is triggerable cross-site by any page through an `<img>`, an iframe or a link prefetch — the class of bug behind CVE-2015-9284 in OmniAuth, and why OmniAuth 2.0 and django-allauth both moved flow initiation to POST. POST alone does not close it (a cross-origin HTML form still submits), so `ensure_trusted_origin` also verifies the initiator: `Sec-Fetch-Site` first (browser-set, unforgeable by page script), falling back to `Origin` against `WebConfig.cors_origins()` for browsers that predate Fetch Metadata, and rejecting with 403 when neither header is present. Both endpoints redirect with **303**, not the 302 authlib hardcodes: RFC 9110 only says a user agent *may* rewrite a redirected POST to GET, and Telegram's authorization endpoint accepts GET only, so `presentation/web/oauth.py` overrides the status on the response authlib returns. The frontend submits these as same-origin `<form method="POST">`, never as links (see [frontend.md](frontend.md)).

  Note the division of labour: authlib owns the *callback* side (`state`, `nonce`, PKCE bound to the session) and has no request-phase CSRF handling at all — that is a web-framework concern, not a protocol-library one, and Starlette ships no CSRF middleware, which is why `csrf.py` exists.
