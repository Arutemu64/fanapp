...
* **Gateways**: Concrete SQL queries, database reads, and inserts are isolated in gateway implementations under `adapters/db/gateways/` (one per aggregate/concern). Each implements its aggregate's abstract `gateways/` port (reads and writes).
