...
* **Shared HTTP client for the delivery notifiers**: the push and VK notifiers are request-scoped (they depend on request-scoped gateways, and push on the uow for pruning), but their outbound HTTP goes through an **APP-scoped, reused** `httpx2.AsyncClient` wrapped in a dedicated client type — `WebPushClient` (`adapters/push/client.py`) and `VkApiClient` (`adapters/vk/client.py`), each built once in its DI provider (`main/ioc/push.py`, `main/ioc/vk.py`). Reusing one client keeps the connection pool warm across a notification burst instead of re-handshaking per message ([httpx guidance](https://www.python-httpx.org/advanced/clients/)); wrapping it in a distinct type keeps a bare `httpx2.AsyncClient` out of the container, where it would be an ambiguous, collision-prone DI key. The notifier keeps only the domain concern — rendering, gateway lookups, and translating the vendor's failures (VK error codes → `UserNotReachable`/`NotificationRetryAfter`; push 404/410 → prune the dead subscription). This mirrors the vendor-client split below (`BaseApiClient`), minus the adaptix DTO loading these two don't need.
* **Two-phase delivery keeps the network send out of the DB transaction**: the `Notifier` port (`application/ports/notifier.py`) is split into `resolve(notification) -> Target` (channel recipient lookup — DB reads only) and `deliver(notification, target)` (the network send). `SendNotification` owns the transaction boundary: it reads the notification, its mailing and the recipient, then **rolls back the read-only transaction before calling `deliver`**, so a pooled DB connection is never held across a webpush/Bot API/VK round-trip — which, fanning out one mailing across the three send subscribers, would otherwise pin a connection per in-flight notification and (before this split) hold the notification's `FOR UPDATE` lock across the send. `deliver` runs outside that transaction; only the push notifier writes, opening a short transaction of its own to prune a dead endpoint. The per-channel `Target` (a `chat_id`, `peer_id`, or list of subscriptions) is opaque to the interactor — it only carries it from `resolve` to `deliver`.
