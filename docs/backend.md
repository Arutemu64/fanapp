...
* **Telegram bot (`presentation/tgbot/`)**: an aiogram 3 `Dispatcher` on **long polling**, run as the `bot` compose service (composition root: `main/tgbot.py`). It is **DM-only** — handlers filter on `F.chat.type == ChatType.PRIVATE`, so group/channel updates never match and the bot stays silent there. Today it has no in-app features: every message and callback gets the same web-app intro plus an inline **URL** button (not a Web App button) built from `WebConfig.build_url("/")`. On startup it drops any webhook, clears the command menu (`delete_my_commands`) and resets the menu button to `MenuButtonDefault` (default scope, all users). The DI-provided `Bot` (`main/ioc/bot.py`, APP scope) is shared with the outbound `TelegramNotifier` (`adapters/tgbot/notifier.py`); dishka's aiogram integration (`setup_dishka(container, dp)`) injects interactors/config into handlers via `@inject` + `FromDishka`.
