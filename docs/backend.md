...
* **Vendor clients** (`adapters/api/<vendor>/client.py`) are thin subclasses of `BaseApiClient` (`adapters/api/base.py`): a small `httpx.AsyncClient` wrapper whose `_get` loads JSON responses into the plain-dataclass vendor DTOs via the injected adaptix `Retort`. Each endpoint is one explicit async method with a real return type — no decorator magic. httpx raises `httpx.HTTPStatusError` on non-2xx (status at `error.response.status_code`); translate vendor errors here (e.g. 404 → `None` in `TCloudClient.get_order`). The client is now a private detail of its source adapter. The base URL and auth headers are configured on the `httpx.AsyncClient` in the DI provider (`main/ioc/<vendor>.py`), which also binds the source to its port.
...
* **Environment posture (`ENV`)**: `ENV=dev|staging|prod` is the single source of truth for the debug posture. `DebugConfig` ships **production-safe defaults** (`enabled=False`, `logging_level=INFO`, `json_logs=True`); `EnvConfig._apply_environment_posture` relaxes them to developer-friendly values (`enabled=True`, `DEBUG`-level, console logs) **only when `ENV=dev`** and the specific `DEBUG__*` var wasn't set explicitly (an explicit value always wins). With `ENV=prod` the app **refuses to start** if `DEBUG__ENABLED=True`, since FastAPI debug mode leaks stack traces in HTTP responses. So leave the derived flags unset in `.env` and just set `ENV`.
