...
def _parse_notification_dto(notification_orm: NotificationORM) -> NotificationDTO:
    return NotificationDTO(
        id=notification_orm.id,
        user_id=notification_orm.user_id,
        title=notification_orm.title,
        body=notification_orm.body,
        type=NotificationType(notification_orm.type),
        mailing_id=notification_orm.mailing_id,
        created_at=notification_orm.created_at,
        seen_at=notification_orm.seen_at,
    )
...
class NotificationGateway:
    ...
    async def add_notification(self, notification: Notification) -> Notification:
        notification_orm = NotificationORM.from_model(notification)
        self.session.add(notification_orm)
        await self.session.flush([notification_orm])
        return notification_orm.to_model()
    ...
    async def get_notification(
        ...
        return notification_orm.to_model() if notification_orm else None
    ...
    async def read_list_user_notifications(
        ...
        return [_parse_notification_dto(n) for n in notifications]
