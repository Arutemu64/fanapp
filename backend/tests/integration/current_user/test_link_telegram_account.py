...
async def test_linking_stores_subject_and_notification_address(
    ...
    interactor = await dishka_request.get(LinkTelegramAccount)
...
async def test_relinking_the_same_telegram_is_a_no_op(
    dishka_request: AsyncContainer,
    uow: UnitOfWork,
    login: Callable[[User], None],
):
...
async def test_linking_a_second_telegram_is_rejected(
    dishka_request: AsyncContainer,
    uow: UnitOfWork,
    login: Callable[[User], None],
):
    ...
    with pytest.raises(UserAlreadyHasTelegramLinked):
        await interactor(
            LinkTelegramAccountInput(
                subject=f"{SUBJECT}9",
                provider_user_id=TELEGRAM_USER_ID + 1,
                initiator_user_id=user.id,
            )
        )


async def test_a_telegram_owned_by_another_user_is_rejected(
    dishka_request: AsyncContainer,
    uow: UnitOfWork,
    login: Callable[[User], None],
):
    ...
    login(newcomer)
    with pytest.raises(TelegramAlreadyLinkedToAnotherUser):
        await interactor(
            LinkTelegramAccountInput(
                subject=SUBJECT,
                provider_user_id=TELEGRAM_USER_ID,
