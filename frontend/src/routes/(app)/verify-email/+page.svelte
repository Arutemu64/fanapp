	...
	<Card class="w-full max-w-md p-6 text-center sm:p-8">
		<div class="flex flex-col items-center justify-center">
			{#if data.status === 'success'}
				...
				<Button href={resolve('/')} class="min-h-11 w-full rounded-xl">Перейти на главную</Button>
			{:else}
				...
				<Button href={resolve('/login')} outline color="dark" class="min-h-11 w-full rounded-xl">
					Вернуться ко входу
				</Button>
			{/if}
		</div>
