<script lang="ts">
	...
	const columns = [
		{
			name: 'number',
			description: 'Номер выступления. Целое число, не должно повторяться.'
		},
		{
			name: 'title',
...
<Card class="mx-auto mb-4 w-full max-w-2xl rounded-2xl p-4 sm:mb-6 sm:p-6">
	...
	<p class="mt-3 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
		Все пять колонок обязательны, пустых ячеек быть не должно. Лишние колонки игнорируются.
	</p>
	...
	<Alert color="yellow" class="mt-4 rounded-lg">
		...
		обновятся, а те, которых в файле нет, будут удалены. Порядок строк задаёт порядок в программе.
	</Alert>
