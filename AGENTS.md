...
* **Command Runner**: `justfile` (run from root):
  ...
  * `just ci` - Run every gate from `.github/workflows/ci.yml` locally, check-only (no auto-fix). Prefer catching failures here over spending an Actions run
...
9. **Keep `.env.example` in Sync**: Added/renamed/removed an env variable in ANY consumer → update `.env.example` in the SAME change, mirroring the file's grouping and comments. `.env.example` is the single source of truth for env config. The backend uses `extra="ignore"`, so a drifted/typo'd key does NOT crash at boot — this rule is the only thing keeping the consumers in sync.
   ...
   * **Frontend** (`frontend/src`, build args): a new `PUBLIC_*` / `VITE_*` read via `$env/static/public` → add it to the FRONTEND block. These are baked in at build time, not runtime. The frontend has no env file of its own — it reads the root `.env` directly (`kit.env.dir` / vite `envDir` point at the repo root; see docs/frontend.md).
...
15. **Minimize Version/Image Drift**: When the same external dependency (Docker image, tool version) is pinned in more than one place, use the *same* pinned version everywhere rather than letting each reference float or drift independently — e.g. `postgres:18.4-alpine` is the one Postgres image for `docker-compose.yml`, `backend/scripts/generate_migration.py`, and `backend/tests/fixtures/db_provider.py`; `uv==0.11.29` is the one uv version for `mise.toml`, `backend/pyproject.toml`, `backend/Dockerfile`, `.claude/setup.sh`, and CI; `hadolint 2.14.0` is the one hadolint version for `mise.toml`, the `.pre-commit-config.yaml` rev, and the image behind the `.claude/setup.sh` shim. Prefer an exact pin over a floating tag so every consumer resolves identically, and bump all references together in the same change — a stray unpinned install (e.g. `pip install --upgrade uv`) or a leftover different tag silently reintroduces drift. If two references genuinely can't share a version (different platform/major requirements), say why in a comment at each site instead of leaving the mismatch unexplained. **Renovate** (`renovate.json`) automates this: shared pins are consolidated into single-PR groups (`uv`, `postgres`, `node`, `pnpm`, `hadolint`) so every reference bumps together, and `customManagers` (regex) cover the pins the built-in managers can't see — the Postgres image literals in `backend/scripts/generate_migration.py` and `backend/tests/fixtures/db_provider.py`; the uv pin in `.claude/setup.sh`, `backend/pyproject.toml` (`[tool.uv]`), and the CI `setup-uv` input; and the pnpm `version` input to `pnpm/action-setup` in CI (kept explicit because v6 can't read `packageManager` from the subdirectory `frontend/package.json` — pnpm/action-setup#227). Third-party GitHub Actions are pinned to commit SHAs (`helpers:pinGitHubActionDigests`) with a `# vX` tag comment. When you add a *new* pin site for an already-tracked dependency, extend the matching group/custom manager in `renovate.json` in the same change, or Renovate will bump the others and silently reintroduce drift. **Rule order matters**: `packageRules` are applied in order and a later rule overrides an earlier one, so the broad CI-minute batching groups (`github actions`, `dev tooling`, `backend dependencies`) must stay *above* the Constraint 15 groups — otherwise a shared pin gets swallowed into the batch PR instead of bumping together in its own.
