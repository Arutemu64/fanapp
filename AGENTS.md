...
* **Never** block on the integration suite (`just backend-test-integration`) — it is slow and CI runs it regardless, so **prefer CI** and never treat work as unfinished for not having run it locally. Running it locally is fine when it earns the time (debugging a failure, Docker permitting); it just isn't a gate. Writing an integration test when the change warrants one is still right. (Cloud web sessions run a Docker daemon — for migration autogenerate via `just backend-generate-auto`, and the suite too if you need it, though CI stays the default. See [docs/claude-cloud.md](docs/claude-cloud.md).)
...
* **Commit & PR titles.** We squash-merge, so the PR title *is* the commit that lands on `main` (GitHub appends `(#N)`). Write it as an imperative, capitalised, period-less sentence ≤~70 chars that names the change **and its effect** in product terms — "Make voting honestly offline-only", not "fix bug". The *why* — the constraint, the rejected alternative — goes in the body / PR description, same as a code comment. Conventional-Commits `type(scope):` prefixes are optional, not enforced (we run no commitlint and no type-driven release automation): use them where they aid scanning of pure noise (`docs:`, `chore:`, Renovate's `Update dependency…`), skip them where a plain product sentence reads better. Pick one per commit and stay consistent within it. When you open a PR, populate [`.github/pull_request_template.md`](.github/pull_request_template.md) — read it and fill its sections from the change; a template only auto-fills in the browser, so a PR created from the CLI or API needs it applied by hand or it lands with a freeform body.
...
* **Research the current best practice before you act — then cite it. This rule is load-bearing; treat it as non-negotiable.** Before *any* non-trivial work — a refactor, a new feature, reaching for a library, an HTTP header, a config default, a caching/security/auth policy, a framework convention — search the web or current docs and decide from what you find. **Never** rely on training memory alone: it lags releases and quietly goes stale. This governs the *action*, not just the claim: verify **before** committing to a direction, and the same standard blocks asserting any best-practice claim you have not just checked. Carry the citation into the PR, the code comment, or your reply. Only genuinely trivial, settled edits — a typo, a rename, a copy tweak — are exempt; when in doubt, look it up.
