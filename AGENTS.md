...
8. **Keep Documentation in Sync**: After any structural, architectural, or path-level change, update `AGENTS.md` and the relevant `docs/*.md` in the same change.
   ...
   * Changed a Core Constraint or command that `.claude/rules/*.md` or `.github/copilot-instructions.md` restates → update those files in the same change (they are thin, path-scoped summaries of this file).
