...
| `just backend-dev` / `just frontend-dev` | Run the API / frontend on the host (pair with `just run-infra`) |
| `just backend-stream` / `just backend-scheduler` | The other two host processes — NATS consumers / outbox relay. Without them domain events silently never deliver (README §3) |
