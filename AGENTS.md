...
This 550+ file codebase is indexed by [`codegraph`](https://www.npmjs.com/package/@colbymchenry/codegraph), a code-intelligence CLI. Not provisioned automatically in any environment (cloud or local) — install once with `pnpm add -g @colbymchenry/codegraph && codegraph init`, then re-run `codegraph sync` after pulling new code. Skip this section entirely if you haven't installed it.
...
8. **Keep Documentation in Sync**: After any structural, architectural, or path-level change, verify and update `AGENTS.md` and relevant `docs/*.md` before marking the task complete.
   ...
   * **Added/renamed/removed an env variable in ANY consumer? Update `.env.example` in the SAME change.** `.env.example` is the single source of truth for env config, serving three consumers — keep whichever one you touched in sync with the matching scope block, and mirror the change in the file's grouping and comments:
     * **Backend** (`adapters/config/models.py`): any add/rename/remove or default/optionality change to a field in `EnvConfig` or a nested `*Config` (`web`, `db`, `redis`, `nats`, `mail`, `bot`, `push`, `debug`, external, `scheduler`, `outbox`, `notification`) → edit the matching `SECTION__FIELD` key in the BACKEND block. The backend uses `extra="ignore"`, so a drifted/typo'd key does NOT crash at boot — this rule is the only thing keeping them in sync. Renaming a generated-secret placeholder? Also update `GENERATED_SECRETS` in `backend/scripts/bootstrap.py` (its drift guard fails loud otherwise).
     ...
     * **Frontend** (`frontend/src`, build args): a new `PUBLIC_*` / `VITE_*` read via `$env/static/public` → add it to the FRONTEND block. Remember these are baked in at build time, not runtime.
