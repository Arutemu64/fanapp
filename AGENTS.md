...
* **Command Runner**: `justfile` (run from root):
  ...
  * `just ci` - Run every gate from `.github/workflows/ci.yml` locally, check-only (no auto-fix). The repo is private, so Actions minutes are billed — prefer catching failures here over spending a CI run
