...
- After changes: run `just backend-lint` and `just backend-typecheck`; fix all errors. Don't run the integration suite (`just backend-test-integration`) — it's slow, so leave it to CI even where Docker is available (cloud web sessions have a daemon — use it for `just backend-generate-auto`, not the integration suite).
