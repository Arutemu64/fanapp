...
- After changes: run `just backend-lint` and `just backend-typecheck`; fix all errors. Don't run the integration suite (`just backend-test-integration`) — it's slow and Docker isn't always present, so leave it to CI.
